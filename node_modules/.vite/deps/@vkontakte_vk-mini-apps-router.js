...
function getMatchesToLoad(history2, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
...
var TransactionExecutor = class _TransactionExecutor {
  ...
  static async doNext() {
    ...
    if (transactions.length) {
      ...
      if ((_a = transactions[0]) == null ? void 0 : _a.finished) {
        transactions.shift();
      }
    }
  }
...
function flattenBranch(leafs, parents) {
  return leafs.map((leaf) => {
    ...
    return leaf.children ? [leafWithParents, ...flattenBranch(leaf.children, [...parents, leafWithParents])] : leafWithParents;
  }).flat();
}
function fillHistory(config, routeNavigator, context) {
  ...
  const targetLeaf = potentialLeafs.find((leaf) => {
    var _a2;
    return !((_a2 = leaf.children) == null ? void 0 : _a2.length);
  }) ?? potentialLeafs[0];
...
var ContextThrottleService = class _ContextThrottleService {
  ...
  constructor() {
    this.interval = 0;
    this.throttled = true;
    this.contextThrottleMap = /* @__PURE__ */ new Map();
  }
  static getInstance() {
...
var DefaultRouteNavigator = class {
  ...
  async showModal(id, options) {
    await this.router.navigate(this.router.state.location, {
      state: {
        ...options == null ? void 0 : options.state,
  ...
  async hideModal(pushPanel = false, options) {
    ...
    if (modalMatch) {
      ...
      invariant2(path, `There is no route registered for panel with ${route.root ? `root: ${route.root} ` : ""}, view: ${route.view}, panel: ${route.panel}. Make sure this route exists or use hideModal with pushPanel set to false.`);
      return await this.navigate(path, {
        keepSearchParams: true,
        replace: Boolean(options == null ? void 0 : options.replace)
      });
    }
    await TransactionExecutor.doNext();
  }
...
var ViewHistory = class {
  ...
  pop(state) {
    this.positionInternal = this.history.findIndex(({ locationKey }) => locationKey === state.location.key);
  }
  hasKey(key) {
    return Boolean(this.history.find(({ locationKey }) => locationKey === key));
  }
  createViewRecord(state, match) {
    var _a;
    ...
    return {
      ...
      params,
      root: "root" in route ? route.root : void 0,
      view: route.view,
      panel: route.panel,
...
// node_modules/@vkontakte/vk-mini-apps-router/dist/services/HistoryManager.js
var HistoryManager = class {
  constructor(viewHistory) {
    this.viewHistory = viewHistory;
  }
  getCurrentPosition() {
...
function useThrottledContext(context) {
  ...
  const [prevValue, setPrevValue] = (0, import_react4.useState)(initialContextData.prevValue);
  const [throttledValue, setThrottledValue] = (0, import_react4.useState)(initialContextData.throttledValue);
...
// node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useHistoryManager.js
var import_react10 = __toESM(require_react());
function useHistoryManager() {
  const { viewHistory } = (0, import_react10.useContext)(RouterContext);
  const historyManager = (0, import_react10.useMemo)(() => new HistoryManager(viewHistory), [viewHistory]);
  invariant2(viewHistory, "You can not use useHistoryManager hook outside of RouteContext. Make sure calling it inside RouterProvider.");
  return historyManager;
}

// node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useParams.js
function getActualRecordParams(id, previousRecord, route) {
  ...
  return currentRouteDoesNotMatchId ? previousRecord.params : void 0;
}
function useParams(id) {
  ...
  const previousRecord = currentPosition ? historyStack[currentPosition - 1] : void 0;
...
function useSearchParams(defaultInit) {
  ...
  let defaultSearchParamsRef = (0, import_react11.useRef)(createSearchParams(defaultInit));
  let hasSetSearchParamsRef = (0, import_react11.useRef)(false);
  let location = useLocation();
  let searchParams = (0, import_react11.useMemo)(() => (
    // Only merge in the defaults if we haven't yet called setSearchParams.
    // Once we call that we want those to take precedence, otherwise you can't
    // remove a param with setSearchParams({}) if it has an initial value
    getSearchParamsForLocation(location.search, hasSetSearchParamsRef.current ? null : defaultSearchParamsRef.current)
  ), [location.search]);
  let router = (0, import_react11.useContext)(RouterContext).router;
  let setSearchParams = (0, import_react11.useCallback)((nextInit, navigateOptions) => {
...
function useFirstPageCheck() {
  const routerContext = (0, import_react12.useContext)(RouterContext);
...
function useEnableSwipeBack() {
  const consumerId = (0, import_react13.useRef)(null);
...
function useBlocker(blocker) {
  ...
  const [routeContext] = useThrottledContext(RouteContext);
  const unblocker = (0, import_react14.useRef)();
